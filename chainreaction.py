...
distinctcoloring= 25
#tries to make colors more distinct ; too big of a value will make no difference at all
  ...
  for event in pygame.event.get():
    if event.type == pygame.QUIT:
      ...
      if play_clicked:
        ...
        player_difference = players-cboxes**2
        ...
        for i in range(players):
          ...
          if alpha==1:
            player_difference = players%cboxes
            for j in range(player_difference):
              nu_deltaa = (screen_x - (player_difference* cbox_width))/(player_difference+1)
